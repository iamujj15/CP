...
bool issafe(ll i, ll j)
{
    ...
    for (ll p = i, q = j; p >= 0 && q >= 0; p--, q--)
        if (grd[p][q] == 1)
            return false;

    for (ll p = i, q = j; p < n && q >= 0; p++, q--)
        if (grd[p][q] == 1)
            return false;
    return true;
}
...
bool qplcrec(ll j)
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        {
            grd[i][j] = 1;
            if (qplcrec(j + 1) == true) return true;
            grd[i][j] = 0;
        }
    }
    return false;
...
void solution()
{
    ...
    // You'll get in output a grid with info. of the locations
    // where you can put your n queens in the grid without letting
    // them attack each other.
    // A Queen can either move in it's row or column or the two diagonals.
    cin >> n;
    grd.resize(n, vll(n));
