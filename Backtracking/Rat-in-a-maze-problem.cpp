...
{
    if (i < n && j < m && mz[i][j] == 1) return true;
    else return false;
}

bool mzslvrec(ll i, ll j)
{
    if (i == n-1 && j == m-1 && mz[i][j] == 1)
    {
        ans[i][j] = 1;
        return true;
    }
    else
    {
        ...
        {
            ans[i][j] = 1;
            if (mzslvrec(i, j+1) == true) return true;
            if (mzslvrec(i+1, j) == true) return true;
            ans[i][j] = 0;
        }
    }
    return false;
